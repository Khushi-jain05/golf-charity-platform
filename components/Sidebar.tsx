// components/Sidebar.tsx  ← update this file
'use client'
...
export default function Sidebar({ profile, onToggle }: SidebarProps) {
const [currentProfile, setCurrentProfile] = useState(profile)
  ...
  return (
    <>
      <aside className={`bg-[#1a1f1a] flex flex-col fixed h-full z-20 transition-all duration-300 ease-in-out
        ${open ? 'w-[240px]' : 'w-[60px]'}`}>
...
<div className="text-[10px] text-green-400/70">
  {currentProfile?.subscription_plan === 'champion'
    ? 'Champion Subscriber'
    : currentProfile?.subscription_plan === 'pro'
    ? 'Pro Subscriber'
    : 'Free Plan'}
</div>
              </div>
          ...
          <button onClick={() => supabase.auth.signOut().then(() => router.push('/'))}
            title="Sign out"
            className="w-full flex items-center justify-center gap-3 px-3 py-2 rounded-lg text-xs text-white/25 hover:text-red-400 transition mt-1">
      ...
      <button
        ...
        className={`fixed top-4 z-30 w-7 h-7 bg-[#1a1f1a] border border-white/10 rounded-full flex items-center justify-center text-white/50 hover:text-white transition-all duration-300
